...
namespace LabWork
{
    ...
    public static class FunctionCalculator
    {
        ...
        public static List<DataPoint> GeneratePoints()
        {
            ...
            // + DELTA_X/2 для гарантованого включення X_END
    ...
    public class PlottingForm : Form
    {
        ...
        public PlottingForm()
        {
            ...
            // ControlStyles: OptimizedDoubleBuffer, AllPaintingInWmPaint, UserPaint - для уникнення миготіння
        ...
        private void AddModeSwitchControls()
        {
            ...
            rbLine.CheckedChanged += (s, e) => {
                ...
                {
                    _isLineMode = true;
                    this.Invalidate(); // Запланувати перерисовку
                }
            };
        ...
        private void PlottingForm_Paint(object sender, PaintEventArgs e)
        {
            ...
            {
                // clientHeight - PADDING - ... забезпечує, що Y=0 буде внизу, а Y збільшується вгору
        ...
        {
            // Y-координата лінії X-осі (де y=0)
            int xAxisY = mapY(0); 
            // X-координата лінії Y-осі (де x=X_MIN)
            ...
            // Мітка X_MIN і Y_MIN
            g.DrawString($"{FunctionCalculator.XMin:F2}", SystemFonts.DefaultFont, Brushes.Gray, yAxisX, clientHeight - PADDING + 5);
        ...
        private void DrawGraph(Graphics g, Func<double, int> mapX, Func<double, int> mapY)
        {
            ...
            for (int i = 0; i < _dataPoints.Count; i++)
            {
                ...
                // Режим: Точковий (або точки для лінійного)
                g.FillEllipse(_pointBrush, 
                    currentPoint.X - markerSize / 2, 
                    currentPoint.Y - markerSize / 2, 
                    markerSize, 
                    markerSize);
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                // Звільняємо ресурси, створені за допомогою 'new Pen(...)'
                _plotPen?.Dispose();
                _axesPen?.Dispose();
                // Примітка: Brushes.Red не потребує Dispose, оскільки це системний об'єкт.
            }
